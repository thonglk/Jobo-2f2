...
angular.module("snackbar", ['ngAnimate']).service('$snackbar', function($http, $log, $animate, $q) {
  ...
  var template = $http({
    method: 'GET',
    url: 'templates/snackbar/snackbar.html'
    ...
    if (previousSnackbar.length == 0) {
      if(navigator.userAgent.indexOf('Mobile') == -1) {
        ...
        angular.element(head).append('<link href="templates/snackbar/snackbar-tablet.css" rel="stylesheet">');
      }
  ...
  this.show = function(options) {
    return $q(function(resolve, reject) {
      ...
      $animate.removeClass(wrapper[0], "active").then(function() {
        ...
        var messageColor = options.messageColor ? options.messageColor : 'black';
