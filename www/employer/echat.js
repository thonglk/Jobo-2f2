...
app.controller('eChatsCtrl', function ($scope, $rootScope, CONFIG, $stateParams, $ionicActionSheet, $timeout, $ionicScrollDelegate, $ionicSlideBoxDelegate, $firebaseArray, $ionicPopup, $http, $ionicLoading) {
  ...
  $scope.chatTo = function (index) {
    ...
    if (index) {
      ...
      var messageRef = firebase.database().ref('chat/' + $rootScope.storeIdCurrent + $rootScope.chatUser.userid);
      messageRef.once('value', function (snap) {
      ...
      messageRef.on('child_added', function (snap) {
        $scope.messages = snap.val();
        console.log('child_add',$scope.messages)
      })
      messageRef.on('child_changed', function (snap) {
        $scope.messages = snap.val();
        console.log('child_changed',$scope.messages)
      })
      messageRef.on('child_removed', function (snap) {
        $scope.messages = snap.val();
        console.log('child_removed',$scope.messages)
      })
